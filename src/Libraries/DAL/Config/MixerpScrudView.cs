...
namespace MixERP.Net.Schemas.Config.Data
{
    public class MixerpScrudView
    {
		...
		public IEnumerable<MixERP.Net.Entities.Config.MixerpScrudView> GetPagedResult(string catalog)
		{
			const string sql = "SELECT * FROM config.mixerp_scrud_view ORDER BY  LIMIT 25 OFFSET 0;";
			return Factory.Get<MixERP.Net.Entities.Config.MixerpScrudView>(catalog, sql);
		}
		...
		public IEnumerable<MixERP.Net.Entities.Config.MixerpScrudView> GetPagedResult(string catalog, long pageNumber)
		{
			long offset = (pageNumber -1) * 25;
			const string sql = "SELECT * FROM config.mixerp_scrud_view ORDER BY  LIMIT 25 OFFSET @0;";
				
			return Factory.Get<MixERP.Net.Entities.Config.MixerpScrudView>(catalog, sql, offset);
		}
	}
}