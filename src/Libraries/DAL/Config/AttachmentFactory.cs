...
namespace MixERP.Net.Schemas.Config.Data
{
    ...
    /// </summary>
    public class AttachmentFactory : DbAccess
    {
        /// <summary>
        ...
        /// </summary>
        public override string ObjectName => "attachment_factory";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}", this.LoginId);
        ...
        public IEnumerable<MixERP.Net.Entities.Config.AttachmentFactory> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM config.attachment_factory ORDER BY key;";
            return Factory.Get<MixERP.Net.Entities.Config.AttachmentFactory>(this.Catalog, sql);
        }
        ...
        public MixERP.Net.Entities.Config.AttachmentFactory Get(string key)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"AttachmentFactory\" filtered by \"Key\" with value {Key} was denied to the user with Login ID {LoginId}", key, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM config.attachment_factory WHERE key=@0;";
            return Factory.Get<MixERP.Net.Entities.Config.AttachmentFactory>(this.Catalog, sql, key).FirstOrDefault();
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Config.AttachmentFactory> Get(string[] keys)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}. keys: {keys}.", this.LoginId, keys);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Config.AttachmentFactory>(this.Catalog, sql, keys);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Config.AttachmentFactory attachmentFactory, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            attachmentFactory.AuditUserId = this.UserId;
            attachmentFactory.AuditTs = System.DateTime.UtcNow;
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Config.AttachmentFactory attachmentFactory)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}. {AttachmentFactory}", this.LoginId, attachmentFactory);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this.Catalog, attachmentFactory);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Config.AttachmentFactory> attachmentFactories)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}. {attachmentFactories}", this.LoginId, attachmentFactories);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            try
            {
                using (Database db = new Database(Factory.GetConnectionString(this.Catalog), Factory.ProviderName))
                {
                    ...
                    {
                        foreach (var attachmentFactory in attachmentFactories)
                        {
                            line++;
                            ...
                            if (!string.IsNullOrWhiteSpace(attachmentFactory.Key))
                            {
                                ...
                                db.Update(attachmentFactory, attachmentFactory.Key);
                            }
                            else
                            {
                                result.Add(db.Insert(attachmentFactory));
                            }
                        }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void Update(MixERP.Net.Entities.Config.AttachmentFactory attachmentFactory, string key)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"AttachmentFactory\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {AttachmentFactory}", key, this.LoginId, attachmentFactory);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this.Catalog, attachmentFactory, key);
        }
        ...
        public void Delete(string key)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"AttachmentFactory\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", key, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Config.AttachmentFactory> GetPagedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}.", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM config.attachment_factory ORDER BY key LIMIT 25 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Config.AttachmentFactory>(this.Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Config.AttachmentFactory> GetPagedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}.", pageNumber, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Config.AttachmentFactory>(this.Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Config.AttachmentFactory> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Config.AttachmentFactory> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"AttachmentFactory\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
