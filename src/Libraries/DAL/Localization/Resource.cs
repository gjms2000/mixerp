...
namespace MixERP.Net.Schemas.Localization.Data
{
    ...
    /// </summary>
    public class Resource : DbAccess
    {
        /// <summary>
        ...
        /// </summary>
        public override string ObjectName => "resources";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"Resource\" was denied to the user with Login ID {LoginId}", this.LoginId);
        ...
        /// <summary>
        /// Executes a select query on the table "localization.resources" to return a all instances of the "Resource" class to export. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Localization.Resource> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"Resource\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM localization.resources ORDER BY resource_id;";
            return Factory.Get<MixERP.Net.Entities.Localization.Resource>(this.Catalog, sql);
        }
        ...
        public MixERP.Net.Entities.Localization.Resource Get(int resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"Resource\" filtered by \"ResourceId\" with value {ResourceId} was denied to the user with Login ID {LoginId}", resourceId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM localization.resources WHERE resource_id=@0;";
            return Factory.Get<MixERP.Net.Entities.Localization.Resource>(this.Catalog, sql, resourceId).FirstOrDefault();
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Localization.Resource> Get(int[] resourceIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"Resource\" was denied to the user with Login ID {LoginId}. resourceIds: {resourceIds}.", this.LoginId, resourceIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Localization.Resource>(this.Catalog, sql, resourceIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"Resource\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"Resource\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Localization.Resource resource, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            {
                primaryKeyValue = resource.ResourceId;
                this.Update(resource, resource.ResourceId);
            }
            else
            {
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Localization.Resource resource)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"Resource\" was denied to the user with Login ID {LoginId}. {Resource}", this.LoginId, resource);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this.Catalog, resource);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Localization.Resource> resources)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"Resource\" was denied to the user with Login ID {LoginId}. {resources}", this.LoginId, resources);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
                    ...
                    using (Transaction transaction = db.GetTransaction())
                    {
                        foreach (var resource in resources)
                        {
                            ...
                            if (resource.ResourceId > 0)
                            {
                                ...
                                db.Update(resource, resource.ResourceId);
                            }
                            else
                            {
                                result.Add(db.Insert(resource));
                            }
                        }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void Update(MixERP.Net.Entities.Localization.Resource resource, int resourceId)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"Resource\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {Resource}", resourceId, this.LoginId, resource);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this.Catalog, resource, resourceId);
        }
        ...
        public void Delete(int resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"Resource\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", resourceId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM localization.resources WHERE resource_id=@0;";
            Factory.NonQuery(this.Catalog, sql, resourceId);
        }

        /// <summary>
        /// Performs a select statement on table "localization.resources" producing a paged result of 25.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Localization.Resource> GetPagedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"Resource\" was denied to the user with Login ID {LoginId}.", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM localization.resources ORDER BY resource_id LIMIT 25 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Localization.Resource>(this.Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Localization.Resource> GetPagedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"Resource\" was denied to the user with Login ID {LoginId}.", pageNumber, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Localization.Resource>(this.Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"Resource\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered select statement on table "localization.resources" producing a paged result of 25.
        /// </summary>
        /// <param name="pageNumber">Enter the page number to produce the paged result.</param>
        /// <param name="filters">The list of filter conditions.</param>
        /// <returns>Returns collection of "Resource" class.</returns>
        ...
        public IEnumerable<MixERP.Net.Entities.Localization.Resource> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"Resource\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"Resource\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Localization.Resource> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"Resource\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
