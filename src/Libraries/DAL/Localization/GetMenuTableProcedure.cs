...
namespace MixERP.Net.Schemas.Localization.Data
{
	...
	public class GetMenuTableProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_menu_table";
        /// <summary>
		...
		public IEnumerable<DbGetMenuTableResult> Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetMenuTableProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM localization.get_menu_table(@0::text);";
			return Factory.Get<DbGetMenuTableResult>(this.Catalog, query, this.CultureCode);
		} 
	}
}