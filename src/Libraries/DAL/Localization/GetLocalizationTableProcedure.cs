...
namespace MixERP.Net.Schemas.Localization.Data
{
	...
	public class GetLocalizationTableProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_localization_table";
        /// <summary>
		...
		public IEnumerable<DbGetLocalizationTableResult> Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetLocalizationTableProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM localization.get_localization_table(@0::text);";
			return Factory.Get<DbGetLocalizationTableResult>(this.Catalog, query, this.CultureCode);
		} 
	}
}