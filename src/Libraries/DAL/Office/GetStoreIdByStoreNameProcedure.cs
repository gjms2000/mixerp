...
namespace MixERP.Net.Schemas.Office.Data
{
	...
	public class GetStoreIdByStoreNameProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_store_id_by_store_name";
        /// <summary>
		...
		public int Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetStoreIdByStoreNameProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM office.get_store_id_by_store_name(@0::text);";
			return Factory.Scalar<int>(this.Catalog, query, this.StoreName);
		} 
	}
}