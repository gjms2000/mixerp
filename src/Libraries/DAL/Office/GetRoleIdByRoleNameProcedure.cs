...
namespace MixERP.Net.Schemas.Office.Data
{
	...
	public class GetRoleIdByRoleNameProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_role_id_by_role_name";
        /// <summary>
		...
		public int Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetRoleIdByRoleNameProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM office.get_role_id_by_role_name(@0::text);";
			return Factory.Scalar<int>(this.Catalog, query, this.RoleName);
		} 
	}
}