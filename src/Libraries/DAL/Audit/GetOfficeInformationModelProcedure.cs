...
namespace MixERP.Net.Schemas.Audit.Data
{
	...
	public class GetOfficeInformationModelProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_office_information_model";
        /// <summary>
		...
		public IEnumerable<DbGetOfficeInformationModelResult> Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetOfficeInformationModelProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM audit.get_office_information_model(@0::integer);";
			return Factory.Get<DbGetOfficeInformationModelResult>(this.Catalog, query, this.PgArg0);
		} 
	}
}