...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class Account : DbAccess
    {
        ...
        /// </summary>
        public override string ObjectName => "accounts";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"Account\" was denied to the user with Login ID {LoginId}", this.LoginId);
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Account> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"Account\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.accounts ORDER BY account_id;";
            return Factory.Get<MixERP.Net.Entities.Core.Account>(this.Catalog, sql);
        }
        ...
        public MixERP.Net.Entities.Core.Account Get(long accountId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"Account\" filtered by \"AccountId\" with value {AccountId} was denied to the user with Login ID {LoginId}", accountId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.accounts WHERE account_id=@0;";
            return Factory.Get<MixERP.Net.Entities.Core.Account>(this.Catalog, sql, accountId).FirstOrDefault();
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Account> Get(long[] accountIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"Account\" was denied to the user with Login ID {LoginId}. accountIds: {accountIds}.", this.LoginId, accountIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.Account>(this.Catalog, sql, accountIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"Account\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"Account\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.Account account, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            account.AuditUserId = this.UserId;
        ...
        public object Add(MixERP.Net.Entities.Core.Account account)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"Account\" was denied to the user with Login ID {LoginId}. {Account}", this.LoginId, account);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this.Catalog, account);
        }
        ...
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.Account> accounts)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"Account\" was denied to the user with Login ID {LoginId}. {accounts}", this.LoginId, accounts);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public void Update(MixERP.Net.Entities.Core.Account account, long accountId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"Account\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {Account}", accountId, this.LoginId, account);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this.Catalog, account, accountId);
        }
        ...
        public void Delete(long accountId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"Account\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", accountId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a select statement on table "core.accounts" producing a paged result of 25.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Account> GetPagedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"Account\" was denied to the user with Login ID {LoginId}.", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.accounts ORDER BY account_id LIMIT 25 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Core.Account>(this.Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Account> GetPagedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"Account\" was denied to the user with Login ID {LoginId}.", pageNumber, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.Account>(this.Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"Account\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered select statement on table "core.accounts" producing a paged result of 25.
        /// </summary>
        /// <param name="pageNumber">Enter the page number to produce the paged result.</param>
        /// <param name="filters">The list of filter conditions.</param>
        /// <returns>Returns collection of "Account" class.</returns>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Account> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"Account\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"Account\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.Account> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"Account\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
