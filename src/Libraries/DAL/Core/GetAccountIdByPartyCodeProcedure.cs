...
namespace MixERP.Net.Schemas.Core.Data
{
	...
	public class GetAccountIdByPartyCodeProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_account_id_by_party_code";
        /// <summary>
		...
		public long Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetAccountIdByPartyCodeProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM core.get_account_id_by_party_code(@0::text);";
			return Factory.Scalar<long>(this.Catalog, query, this.PartyCode);
		} 
	}
}