...
namespace MixERP.Net.Schemas.Core.Data
{
	...
	public class CreateCardTypeProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "create_card_type";
        /// <summary>
		...
		public void Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"CreateCardTypeProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM core.create_card_type(@0::integer, @1::character varying, @2::character varying);";
			Factory.NonQuery(this.Catalog, query, this.CardTypeId, this.CardTypeCode, this.CardTypeName);
		} 
	}
}