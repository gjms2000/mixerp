...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class ShippingAddress : DbAccess
    {
        ...
        /// <summary>
        /// Executes a select query on the table "core.shipping_addresses" to return a all instances of the "ShippingAddress" class to export. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ShippingAddress> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ShippingAddress> Get(long[] shippingAddressIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. shippingAddressIds: {shippingAddressIds}.", this.LoginId, shippingAddressIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.ShippingAddress>(this.Catalog, sql, shippingAddressIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.ShippingAddress shippingAddress, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            shippingAddress.AuditUserId = this.UserId;
            shippingAddress.AuditTs = System.DateTime.UtcNow;

            if (shippingAddress.ShippingAddressId > 0)
            {
                primaryKeyValue = shippingAddress.ShippingAddressId;
                this.Update(shippingAddress, shippingAddress.ShippingAddressId);
            }
            else
            {
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Core.ShippingAddress shippingAddress)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            return Factory.Insert(this.Catalog, shippingAddress);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.ShippingAddress> shippingAddresses)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. {shippingAddresses}", this.LoginId, shippingAddresses);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
                ...
                using (Database db = new Database(Factory.GetConnectionString(this.Catalog), Factory.ProviderName))
                {
                    using (Transaction transaction = db.GetTransaction())
                    {
                        foreach (var shippingAddress in shippingAddresses)
                        {
                            ...
                            if (shippingAddress.ShippingAddressId > 0)
                            {
                                result.Add(shippingAddress.ShippingAddressId);
                                db.Update(shippingAddress, shippingAddress.ShippingAddressId);
                            }
                            else
                            {
                                result.Add(db.Insert(shippingAddress));
                            }
                        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ShippingAddress> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"ShippingAddress\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
