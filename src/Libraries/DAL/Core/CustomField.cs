...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class CustomField : DbAccess
    {
        ...
        /// </summary>
        public override string ObjectName => "custom_fields";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"CustomField\" was denied to the user with Login ID {LoginId}", this.LoginId);
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CustomField> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"CustomField\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.custom_fields ORDER BY custom_field_id;";
            return Factory.Get<MixERP.Net.Entities.Core.CustomField>(this.Catalog, sql);
        }
        ...
        public MixERP.Net.Entities.Core.CustomField Get(long customFieldId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"CustomField\" filtered by \"CustomFieldId\" with value {CustomFieldId} was denied to the user with Login ID {LoginId}", customFieldId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.custom_fields WHERE custom_field_id=@0;";
            return Factory.Get<MixERP.Net.Entities.Core.CustomField>(this.Catalog, sql, customFieldId).FirstOrDefault();
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CustomField> Get(long[] customFieldIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"CustomField\" was denied to the user with Login ID {LoginId}. customFieldIds: {customFieldIds}.", this.LoginId, customFieldIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.CustomField>(this.Catalog, sql, customFieldIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"CustomField\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"CustomField\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.CustomField customField, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
        ...
        public object Add(MixERP.Net.Entities.Core.CustomField customField)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"CustomField\" was denied to the user with Login ID {LoginId}. {CustomField}", this.LoginId, customField);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this.Catalog, customField);
        }
        ...
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.CustomField> customFields)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"CustomField\" was denied to the user with Login ID {LoginId}. {customFields}", this.LoginId, customFields);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public void Update(MixERP.Net.Entities.Core.CustomField customField, long customFieldId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"CustomField\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {CustomField}", customFieldId, this.LoginId, customField);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this.Catalog, customField, customFieldId);
        }
        ...
        public void Delete(long customFieldId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"CustomField\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", customFieldId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM core.custom_fields WHERE custom_field_id=@0;";
            Factory.NonQuery(this.Catalog, sql, customFieldId);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CustomField> GetPagedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"CustomField\" was denied to the user with Login ID {LoginId}.", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.custom_fields ORDER BY custom_field_id LIMIT 25 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Core.CustomField>(this.Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CustomField> GetPagedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"CustomField\" was denied to the user with Login ID {LoginId}.", pageNumber, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.CustomField>(this.Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"CustomField\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CustomField> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"CustomField\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"CustomField\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CustomField> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"CustomField\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
