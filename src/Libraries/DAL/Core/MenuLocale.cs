...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class MenuLocale : DbAccess
    {
        ...
        /// <summary>
        /// Executes a select query on the table "core.menu_locale" to return a all instances of the "MenuLocale" class to export. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.MenuLocale> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"MenuLocale\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.MenuLocale> Get(int[] menuLocaleIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"MenuLocale\" was denied to the user with Login ID {LoginId}. menuLocaleIds: {menuLocaleIds}.", this.LoginId, menuLocaleIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.MenuLocale menuLocale, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            if (menuLocale.MenuLocaleId > 0)
            {
                primaryKeyValue = menuLocale.MenuLocaleId;
                this.Update(menuLocale, menuLocale.MenuLocaleId);
            }
            else
            {
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Core.MenuLocale menuLocale)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            return Factory.Insert(this.Catalog, menuLocale);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.MenuLocale> menuLocales)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"MenuLocale\" was denied to the user with Login ID {LoginId}. {menuLocales}", this.LoginId, menuLocales);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            try
            {
                using (Database db = new Database(Factory.GetConnectionString(this.Catalog), Factory.ProviderName))
                {
                    ...
                    {
                        foreach (var menuLocale in menuLocales)
                        {
                            line++;
                            ...
                            if (menuLocale.MenuLocaleId > 0)
                            {
                                result.Add(menuLocale.MenuLocaleId);
                                db.Update(menuLocale, menuLocale.MenuLocaleId);
                            }
                            else
                            {
                                result.Add(db.Insert(menuLocale));
                            }
                        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"MenuLocale\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"MenuLocale\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.MenuLocale> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"MenuLocale\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
