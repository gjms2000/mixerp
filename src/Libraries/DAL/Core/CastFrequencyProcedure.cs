...
namespace MixERP.Net.Schemas.Core.Data
{
	...
	public class CastFrequencyProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "cast_frequency";
        /// <summary>
		...
		public int Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"CastFrequencyProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM core.cast_frequency(@0::text);";
			return Factory.Scalar<int>(this.Catalog, query, this.FrequencyCode);
		} 
	}
}