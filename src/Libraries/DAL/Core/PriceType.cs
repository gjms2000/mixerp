...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class PriceType : DbAccess
    {
        ...
        /// <summary>
        /// Executes a select query on the table "core.price_types" to return a all instances of the "PriceType" class to export. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.PriceType> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"PriceType\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.PriceType priceType, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            priceType.AuditUserId = this.UserId;
            priceType.AuditTs = System.DateTime.UtcNow;

            if (priceType.PriceTypeId > 0)
            {
                primaryKeyValue = priceType.PriceTypeId;
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Core.PriceType priceType)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            return Factory.Insert(this.Catalog, priceType);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.PriceType> priceTypes)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"PriceType\" was denied to the user with Login ID {LoginId}. {priceTypes}", this.LoginId, priceTypes);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
                ...
                using (Database db = new Database(Factory.GetConnectionString(this.Catalog), Factory.ProviderName))
                {
                    using (Transaction transaction = db.GetTransaction())
                    {
                        foreach (var priceType in priceTypes)
                        {
                            ...
                            if (priceType.PriceTypeId > 0)
                            {
                                result.Add(priceType.PriceTypeId);
                                db.Update(priceType, priceType.PriceTypeId);
                            }
                            else
                            {
                                result.Add(db.Insert(priceType));
                            }
                        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"PriceType\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"PriceType\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.PriceType> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"PriceType\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
