...
namespace MixERP.Net.Schemas.Core.Data
{
	...
	public class GetCurrencyCodeByPartyIdProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_currency_code_by_party_id";
        /// <summary>
		...
		public string Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetCurrencyCodeByPartyIdProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM core.get_currency_code_by_party_id(@0::bigint);";
			return Factory.Scalar<string>(this.Catalog, query, this.PartyId);
		} 
	}
}