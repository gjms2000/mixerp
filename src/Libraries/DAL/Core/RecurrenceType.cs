...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class RecurrenceType : DbAccess
    {
        ...
        /// <summary>
        /// Executes a select query on the table "core.recurrence_types" to return a all instances of the "RecurrenceType" class to export. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.RecurrenceType> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"RecurrenceType\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.RecurrenceType> Get(int[] recurrenceTypeIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"RecurrenceType\" was denied to the user with Login ID {LoginId}. recurrenceTypeIds: {recurrenceTypeIds}.", this.LoginId, recurrenceTypeIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.RecurrenceType>(this.Catalog, sql, recurrenceTypeIds);
        }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.RecurrenceType recurrenceType, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            recurrenceType.AuditUserId = this.UserId;
            recurrenceType.AuditTs = System.DateTime.UtcNow;

            if (recurrenceType.RecurrenceTypeId > 0)
            {
                primaryKeyValue = recurrenceType.RecurrenceTypeId;
                this.Update(recurrenceType, recurrenceType.RecurrenceTypeId);
            }
            else
            {
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Core.RecurrenceType recurrenceType)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            return Factory.Insert(this.Catalog, recurrenceType);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.RecurrenceType> recurrenceTypes)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"RecurrenceType\" was denied to the user with Login ID {LoginId}. {recurrenceTypes}", this.LoginId, recurrenceTypes);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
                ...
                using (Database db = new Database(Factory.GetConnectionString(this.Catalog), Factory.ProviderName))
                {
                    using (Transaction transaction = db.GetTransaction())
                    {
                        foreach (var recurrenceType in recurrenceTypes)
                        {
                            ...
                            if (recurrenceType.RecurrenceTypeId > 0)
                            {
                                result.Add(recurrenceType.RecurrenceTypeId);
                                db.Update(recurrenceType, recurrenceType.RecurrenceTypeId);
                            }
                            else
                            {
                                result.Add(db.Insert(recurrenceType));
                            }
                        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"RecurrenceType\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"RecurrenceType\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.RecurrenceType> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"RecurrenceType\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
