...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class CashFlowHeading : DbAccess
    {
        ...
        /// <summary>
        /// Executes a select query on the table "core.cash_flow_headings" to return a all instances of the "CashFlowHeading" class to export. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CashFlowHeading> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"CashFlowHeading\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CashFlowHeading> Get(int[] cashFlowHeadingIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"CashFlowHeading\" was denied to the user with Login ID {LoginId}. cashFlowHeadingIds: {cashFlowHeadingIds}.", this.LoginId, cashFlowHeadingIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.CashFlowHeading>(this.Catalog, sql, cashFlowHeadingIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"CashFlowHeading\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.CashFlowHeading cashFlowHeading, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            cashFlowHeading.AuditUserId = this.UserId;
            cashFlowHeading.AuditTs = System.DateTime.UtcNow;

            if (cashFlowHeading.CashFlowHeadingId > 0)
            {
                primaryKeyValue = cashFlowHeading.CashFlowHeadingId;
                this.Update(cashFlowHeading, cashFlowHeading.CashFlowHeadingId);
            }
            else
            {
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Core.CashFlowHeading cashFlowHeading)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            return Factory.Insert(this.Catalog, cashFlowHeading);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.CashFlowHeading> cashFlowHeadings)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"CashFlowHeading\" was denied to the user with Login ID {LoginId}. {cashFlowHeadings}", this.LoginId, cashFlowHeadings);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            try
            {
                using (Database db = new Database(Factory.GetConnectionString(this.Catalog), Factory.ProviderName))
                {
                    ...
                    {
                        foreach (var cashFlowHeading in cashFlowHeadings)
                        {
                            line++;
                            ...
                            if (cashFlowHeading.CashFlowHeadingId > 0)
                            {
                                result.Add(cashFlowHeading.CashFlowHeadingId);
                                db.Update(cashFlowHeading, cashFlowHeading.CashFlowHeadingId);
                            }
                            else
                            {
                                result.Add(db.Insert(cashFlowHeading));
                            }
                        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"CashFlowHeading\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"CashFlowHeading\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.CashFlowHeading> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"CashFlowHeading\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
