...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    public class ZipCodeType : DbAccess
    {
        ...
        /// <summary>
        /// Executes a select query on the table "core.zip_code_types" to return a all instances of the "ZipCodeType" class to export. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ZipCodeType> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"ZipCodeType\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.ZipCodeType zipCodeType, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            zipCodeType.AuditUserId = this.UserId;
            zipCodeType.AuditTs = System.DateTime.UtcNow;

            if (zipCodeType.ZipCodeTypeId > 0)
            {
                primaryKeyValue = zipCodeType.ZipCodeTypeId;
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Core.ZipCodeType zipCodeType)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            return Factory.Insert(this.Catalog, zipCodeType);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.ZipCodeType> zipCodeTypes)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"ZipCodeType\" was denied to the user with Login ID {LoginId}. {zipCodeTypes}", this.LoginId, zipCodeTypes);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            try
            {
                using (Database db = new Database(Factory.GetConnectionString(this.Catalog), Factory.ProviderName))
                {
                    ...
                    {
                        foreach (var zipCodeType in zipCodeTypes)
                        {
                            line++;
                            ...
                            if (zipCodeType.ZipCodeTypeId > 0)
                            {
                                result.Add(zipCodeType.ZipCodeTypeId);
                                db.Update(zipCodeType, zipCodeType.ZipCodeTypeId);
                            }
                            else
                            {
                                result.Add(db.Insert(zipCodeType));
                            }
                        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ZipCodeType\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"ZipCodeType\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.ZipCodeType> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"ZipCodeType\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
