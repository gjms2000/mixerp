    ...
    public class BankAccount : DbAccess
    {
        ...
        public IEnumerable<MixERP.Net.Entities.Core.BankAccount> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"BankAccount\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.BankAccount bankAccount, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            bankAccount.AuditUserId = this.UserId;
            bankAccount.AuditTs = System.DateTime.UtcNow;

            if (bankAccount.AccountId > 0)
            {
                primaryKeyValue = bankAccount.AccountId;
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Core.BankAccount bankAccount)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            return Factory.Insert(this.Catalog, bankAccount);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.BankAccount> bankAccounts)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"BankAccount\" was denied to the user with Login ID {LoginId}. {bankAccounts}", this.LoginId, bankAccounts);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
                ...
                using (Database db = new Database(Factory.GetConnectionString(this.Catalog), Factory.ProviderName))
                {
                    using (Transaction transaction = db.GetTransaction())
                    {
                        foreach (var bankAccount in bankAccounts)
                        {
                            ...
                            if (bankAccount.AccountId > 0)
                            {
                                result.Add(bankAccount.AccountId);
                                db.Update(bankAccount, bankAccount.AccountId);
                            }
                            else
                            {
                                result.Add(db.Insert(bankAccount));
                            }
                        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"BankAccount\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"BankAccount\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.BankAccount> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"BankAccount\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
