    ...
    public class RecurringInvoiceSetup : DbAccess
    {
        ...
        public IEnumerable<MixERP.Net.Entities.Core.RecurringInvoiceSetup> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"RecurringInvoiceSetup\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.RecurringInvoiceSetup> Get(int[] recurringInvoiceSetupIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"RecurringInvoiceSetup\" was denied to the user with Login ID {LoginId}. recurringInvoiceSetupIds: {recurringInvoiceSetupIds}.", this.LoginId, recurringInvoiceSetupIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.RecurringInvoiceSetup>(this.Catalog, sql, recurringInvoiceSetupIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"RecurringInvoiceSetup\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.RecurringInvoiceSetup recurringInvoiceSetup, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            recurringInvoiceSetup.AuditUserId = this.UserId;
        ...
        public object Add(MixERP.Net.Entities.Core.RecurringInvoiceSetup recurringInvoiceSetup)
        {
            ...
            return Factory.Insert(this.Catalog, recurringInvoiceSetup);
        }
        ...
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.RecurringInvoiceSetup> recurringInvoiceSetups)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"RecurringInvoiceSetup\" was denied to the user with Login ID {LoginId}. {recurringInvoiceSetups}", this.LoginId, recurringInvoiceSetups);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"RecurringInvoiceSetup\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"RecurringInvoiceSetup\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.RecurringInvoiceSetup> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"RecurringInvoiceSetup\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
