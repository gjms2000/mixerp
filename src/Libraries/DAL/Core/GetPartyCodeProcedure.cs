...
namespace MixERP.Net.Schemas.Core.Data
{
	...
	public class GetPartyCodeProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_party_code";
        /// <summary>
		...
		public string Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetPartyCodeProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM core.get_party_code(@0::text, @1::text, @2::text);";
			return Factory.Scalar<string>(this.Catalog, query, this.PgArg0, this.PgArg1, this.PgArg2);
		} 
	}
}