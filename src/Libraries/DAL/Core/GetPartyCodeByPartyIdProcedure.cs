...
namespace MixERP.Net.Schemas.Core.Data
{
	...
	public class GetPartyCodeByPartyIdProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_party_code_by_party_id";
        /// <summary>
		...
		public string Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetPartyCodeByPartyIdProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM core.get_party_code_by_party_id(@0::bigint);";
