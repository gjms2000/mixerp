...
namespace MixERP.Net.Schemas.Core.Data
{
	...
	public class RecreateMenuProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "recreate_menu";
        /// <summary>
		...
		public void Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"RecreateMenuProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM core.recreate_menu(@0::text, @1::text, @2::text, @3::integer, @4::integer);";
			Factory.NonQuery(this.Catalog, query, this.MenuText, this.Url, this.MenuCode, this.Level, this.ParentMenuId);
		} 
	}
}