...
namespace MixERP.Net.Schemas.Core.Data
{
	...
	public class GetRecurringAmountByRecurringInvoiceIdProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_recurring_amount_by_recurring_invoice_id";
        /// <summary>
		...
		public decimal Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetRecurringAmountByRecurringInvoiceIdProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM core.get_recurring_amount_by_recurring_invoice_id(@0::integer);";
			return Factory.Get<decimal>(this.Catalog, query, this.RecurringInvoiceId).FirstOrDefault();
		} 
	}
}