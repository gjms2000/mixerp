...
namespace MixERP.Net.Schemas.Core.Data
{
	...
	public class AppendIfNotNullProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "append_if_not_null";
        /// <summary>
		...
		public string Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"AppendIfNotNullProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM core.append_if_not_null(@0::text, @1::text);";
			return Factory.Scalar<string>(this.Catalog, query, this.Source, this.ToAppend);
		} 
	}
}