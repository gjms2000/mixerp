...
namespace MixERP.Net.Schemas.Core.Data
{
    ...
    /// </summary>
    public class FlagType : DbAccess
    {
        /// <summary>
        ...
        /// </summary>
        public override string ObjectName => "flag_types";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"FlagType\" was denied to the user with Login ID {LoginId}", this.LoginId);
        ...
        /// <summary>
        /// Executes a select query on the table "core.flag_types" to return a all instances of the "FlagType" class to export. 
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.FlagType> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"FlagType\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.flag_types ORDER BY flag_type_id;";
            return Factory.Get<MixERP.Net.Entities.Core.FlagType>(this.Catalog, sql);
        }
        ...
        public MixERP.Net.Entities.Core.FlagType Get(int flagTypeId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"FlagType\" filtered by \"FlagTypeId\" with value {FlagTypeId} was denied to the user with Login ID {LoginId}", flagTypeId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.flag_types WHERE flag_type_id=@0;";
            return Factory.Get<MixERP.Net.Entities.Core.FlagType>(this.Catalog, sql, flagTypeId).FirstOrDefault();
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.FlagType> Get(int[] flagTypeIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"FlagType\" was denied to the user with Login ID {LoginId}. flagTypeIds: {flagTypeIds}.", this.LoginId, flagTypeIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.FlagType>(this.Catalog, sql, flagTypeIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"FlagType\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"FlagType\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Core.FlagType flagType, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            flagType.AuditUserId = this.UserId;
            flagType.AuditTs = System.DateTime.UtcNow;

            if (flagType.FlagTypeId > 0)
            {
                primaryKeyValue = flagType.FlagTypeId;
                this.Update(flagType, flagType.FlagTypeId);
            }
            else
            {
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object Add(MixERP.Net.Entities.Core.FlagType flagType)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return null;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"FlagType\" was denied to the user with Login ID {LoginId}. {FlagType}", this.LoginId, flagType);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this.Catalog, flagType);
        }
        ...
        /// <returns></returns>
        public List<object> BulkImport(List<MixERP.Net.Entities.Core.FlagType> flagTypes)
        {
            if (!this.SkipValidation)
            {
                if (!this.Validated)
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"FlagType\" was denied to the user with Login ID {LoginId}. {flagTypes}", this.LoginId, flagTypes);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
                ...
                using (Database db = new Database(Factory.GetConnectionString(this.Catalog), Factory.ProviderName))
                {
                    using (Transaction transaction = db.GetTransaction())
                    {
                        foreach (var flagType in flagTypes)
                        {
                            ...
                            if (flagType.FlagTypeId > 0)
                            {
                                result.Add(flagType.FlagTypeId);
                                db.Update(flagType, flagType.FlagTypeId);
                            }
                            else
                            {
                                result.Add(db.Insert(flagType));
                            }
                        }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void Update(MixERP.Net.Entities.Core.FlagType flagType, int flagTypeId)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"FlagType\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {FlagType}", flagTypeId, this.LoginId, flagType);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this.Catalog, flagType, flagTypeId);
        }
        ...
        public void Delete(int flagTypeId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"FlagType\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", flagTypeId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM core.flag_types WHERE flag_type_id=@0;";
            Factory.NonQuery(this.Catalog, sql, flagTypeId);
        }

        /// <summary>
        /// Performs a select statement on table "core.flag_types" producing a paged result of 25.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.FlagType> GetPagedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"FlagType\" was denied to the user with Login ID {LoginId}.", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM core.flag_types ORDER BY flag_type_id LIMIT 25 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.Core.FlagType>(this.Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.FlagType> GetPagedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"FlagType\" was denied to the user with Login ID {LoginId}.", pageNumber, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Core.FlagType>(this.Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"FlagType\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered select statement on table "core.flag_types" producing a paged result of 25.
        /// </summary>
        /// <param name="pageNumber">Enter the page number to produce the paged result.</param>
        /// <param name="filters">The list of filter conditions.</param>
        /// <returns>Returns collection of "FlagType" class.</returns>
        ...
        public IEnumerable<MixERP.Net.Entities.Core.FlagType> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"FlagType\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"FlagType\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Core.FlagType> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"FlagType\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
