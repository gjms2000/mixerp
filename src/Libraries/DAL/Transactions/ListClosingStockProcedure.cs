...
namespace MixERP.Net.Schemas.Transactions.Data
{
	...
	public class ListClosingStockProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "list_closing_stock";
        /// <summary>
		...
		public IEnumerable<DbListClosingStockResult> Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"ListClosingStockProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM transactions.list_closing_stock(@0::integer);";
			return Factory.Get<DbListClosingStockResult>(this.Catalog, query, this.StoreId);
		} 
	}
}