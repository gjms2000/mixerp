    ...
    public class StockDetail : DbAccess
    {
        ...
        public IEnumerable<MixERP.Net.Entities.Transactions.StockDetail> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"StockDetail\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Transactions.StockDetail> Get(long[] stockDetailIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"StockDetail\" was denied to the user with Login ID {LoginId}. stockDetailIds: {stockDetailIds}.", this.LoginId, stockDetailIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.Transactions.StockDetail>(this.Catalog, sql, stockDetailIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"StockDetail\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.Transactions.StockDetail stockDetail, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            stockDetail.AuditUserId = this.UserId;
        ...
        public object Add(MixERP.Net.Entities.Transactions.StockDetail stockDetail)
        {
            ...
            return Factory.Insert(this.Catalog, stockDetail);
        }
        ...
        public List<object> BulkImport(List<MixERP.Net.Entities.Transactions.StockDetail> stockDetails)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"StockDetail\" was denied to the user with Login ID {LoginId}. {stockDetails}", this.LoginId, stockDetails);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"StockDetail\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"StockDetail\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.Transactions.StockDetail> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"StockDetail\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
