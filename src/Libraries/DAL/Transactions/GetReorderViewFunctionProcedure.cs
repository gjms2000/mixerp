...
namespace MixERP.Net.Schemas.Transactions.Data
{
	...
	public class GetReorderViewFunctionProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_reorder_view_function";
        /// <summary>
		...
		public IEnumerable<DbGetReorderViewFunctionResult> Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetReorderViewFunctionProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM transactions.get_reorder_view_function(@0::integer);";
			return Factory.Get<DbGetReorderViewFunctionResult>(this.Catalog, query, this.OfficeId);
		} 
	}
}