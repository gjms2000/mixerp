...
namespace MixERP.Net.Schemas.Transactions.Data
{
	...
	public class GetInvoiceAmountProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_invoice_amount";
        /// <summary>
		...
		public decimal Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetInvoiceAmountProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM transactions.get_invoice_amount(@0::bigint);";
			return Factory.Get<decimal>(this.Catalog, query, this.TransactionMasterId).FirstOrDefault();
		} 
	}
}