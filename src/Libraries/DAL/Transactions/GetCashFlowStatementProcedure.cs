...
namespace MixERP.Net.Schemas.Transactions.Data
{
	...
	public class GetCashFlowStatementProcedure: DbAccess
	{
        ...
        /// </summary>
	    public override string ObjectName => "get_cash_flow_statement";
        /// <summary>
		...
		public string Execute()
		{
			if (!this.SkipValidation)
			{
				...
				if (!this.HasAccess)
				{
                    Log.Information("Access to the function \"GetCashFlowStatementProcedure\" was denied to the user with Login ID {LoginId}.", this.LoginId);
					throw new UnauthorizedException("Access is denied.");
				}
			}
			const string query = "SELECT * FROM transactions.get_cash_flow_statement(@0::date, @1::date, @2::integer, @3::integer, @4::integer);";
			return Factory.Scalar<string>(this.Catalog, query, this.DateFrom, this.DateTo, this.UserId, this.OfficeId, this.Factor);
		} 
	}
}