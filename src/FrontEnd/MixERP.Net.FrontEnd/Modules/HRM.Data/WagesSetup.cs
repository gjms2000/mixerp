...
namespace MixERP.Net.Core.Modules.HRM.Data
{
    ...
    public class WagesSetup : DbAccess
    {
        ...
        /// </summary>
        public override string ObjectName => "wages_setup";
        ...
        public long Count()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"WagesSetup\" was denied to the user with Login ID {LoginId}", this.LoginId);
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.WagesSetup> Get()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the export entity \"WagesSetup\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.wages_setup ORDER BY wages_setup_id;";
            return Factory.Get<MixERP.Net.Entities.HRM.WagesSetup>(this.Catalog, sql);
        }
        ...
        public MixERP.Net.Entities.HRM.WagesSetup Get(int wagesSetupId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the get entity \"WagesSetup\" filtered by \"WagesSetupId\" with value {WagesSetupId} was denied to the user with Login ID {LoginId}", wagesSetupId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.wages_setup WHERE wages_setup_id=@0;";
            return Factory.Get<MixERP.Net.Entities.HRM.WagesSetup>(this.Catalog, sql, wagesSetupId).FirstOrDefault();
        }
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.WagesSetup> Get(int[] wagesSetupIds)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to entity \"WagesSetup\" was denied to the user with Login ID {LoginId}. wagesSetupIds: {wagesSetupIds}.", this.LoginId, wagesSetupIds);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.HRM.WagesSetup>(this.Catalog, sql, wagesSetupIds);
        }
        ...
        public IEnumerable<PetaPoco.CustomField> GetCustomFields(string resourceId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get custom fields for entity \"WagesSetup\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<DisplayField> GetDisplayFields()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to get display field for entity \"WagesSetup\" was denied to the user with Login ID {LoginId}", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public void AddOrEdit(MixERP.Net.Entities.HRM.WagesSetup wagesSetup, List<EntityParser.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this.Catalog))
            {
                return;
            ...
            wagesSetup.AuditUserId = this.UserId;
        ...
        public object Add(MixERP.Net.Entities.HRM.WagesSetup wagesSetup)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to add entity \"WagesSetup\" was denied to the user with Login ID {LoginId}. {WagesSetup}", this.LoginId, wagesSetup);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            return Factory.Insert(this.Catalog, wagesSetup);
        }
        ...
        public List<object> BulkImport(List<MixERP.Net.Entities.HRM.WagesSetup> wagesSetups)
        {
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to import entity \"WagesSetup\" was denied to the user with Login ID {LoginId}. {wagesSetups}", this.LoginId, wagesSetups);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public void Update(MixERP.Net.Entities.HRM.WagesSetup wagesSetup, int wagesSetupId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to edit entity \"WagesSetup\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}. {WagesSetup}", wagesSetupId, this.LoginId, wagesSetup);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            Factory.Update(this.Catalog, wagesSetup, wagesSetupId);
        }
        ...
        public void Delete(int wagesSetupId)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to delete entity \"WagesSetup\" with Primary Key {PrimaryKey} was denied to the user with Login ID {LoginId}.", wagesSetupId, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "DELETE FROM hrm.wages_setup WHERE wages_setup_id=@0;";
            Factory.NonQuery(this.Catalog, sql, wagesSetupId);
        }

        /// <summary>
        /// Performs a select statement on table "hrm.wages_setup" producing a paged result of 25.
        /// </summary>
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.WagesSetup> GetPagedResult()
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to the first page of the entity \"WagesSetup\" was denied to the user with Login ID {LoginId}.", this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }

            const string sql = "SELECT * FROM hrm.wages_setup ORDER BY wages_setup_id LIMIT 25 OFFSET 0;";
            return Factory.Get<MixERP.Net.Entities.HRM.WagesSetup>(this.Catalog, sql);
        }
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.WagesSetup> GetPagedResult(long pageNumber)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the entity \"WagesSetup\" was denied to the user with Login ID {LoginId}.", pageNumber, this.LoginId);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
            ...
            return Factory.Get<MixERP.Net.Entities.HRM.WagesSetup>(this.Catalog, sql, offset);
        }
        ...
        public long CountWhere(List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"WagesSetup\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        /// <summary>
        /// Performs a filtered select statement on table "hrm.wages_setup" producing a paged result of 25.
        /// </summary>
        /// <param name="pageNumber">Enter the page number to produce the paged result.</param>
        /// <param name="filters">The list of filter conditions.</param>
        /// <returns>Returns collection of "WagesSetup" class.</returns>
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.WagesSetup> GetWhere(long pageNumber, List<EntityParser.Filter> filters)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"WagesSetup\" was denied to the user with Login ID {LoginId}. Filters: {Filters}.", pageNumber, this.LoginId, filters);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public long CountFiltered(string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to count entity \"WagesSetup\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
        ...
        public IEnumerable<MixERP.Net.Entities.HRM.WagesSetup> GetFiltered(long pageNumber, string filterName)
        {
            ...
            if (!this.SkipValidation)
            {
                ...
                if (!this.HasAccess)
                {
                    Log.Information("Access to Page #{Page} of the filtered entity \"WagesSetup\" was denied to the user with Login ID {LoginId}. Filter: {Filter}.", pageNumber, this.LoginId, filterName);
                    throw new UnauthorizedException("Access is denied.");
                }
            }
