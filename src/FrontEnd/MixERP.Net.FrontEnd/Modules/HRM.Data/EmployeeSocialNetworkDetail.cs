...
namespace MixERP.Net.Core.Modules.HRM.Data
{
    public class EmployeeSocialNetworkDetail
    {
		...
		public MixERP.Net.Entities.HRM.EmployeeSocialNetworkDetail Get(string catalog, long employeeSocialNetworkDetailId)
		{
			const string sql = "SELECT * FROM hrm.employee_social_network_details WHERE employee_social_network_detail_id=@0;";
			return Factory.Get<MixERP.Net.Entities.HRM.EmployeeSocialNetworkDetail>(catalog, sql, employeeSocialNetworkDetailId).FirstOrDefault();
		}
		...
		public void Add(string catalog, MixERP.Net.Entities.HRM.EmployeeSocialNetworkDetail employeeSocialNetworkDetail)
		{
			Factory.Insert(catalog, employeeSocialNetworkDetail);
		}
		...
		public void Update(string catalog, MixERP.Net.Entities.HRM.EmployeeSocialNetworkDetail employeeSocialNetworkDetail, long employeeSocialNetworkDetailId)
		{
			Factory.Update(catalog, employeeSocialNetworkDetail, employeeSocialNetworkDetailId);
		}
		...
		public void Delete(string catalog, long employeeSocialNetworkDetailId)
		{
			const string sql = "DELETE FROM hrm.employee_social_network_details WHERE employee_social_network_detail_id=@0;";
			Factory.NonQuery(catalog, sql, employeeSocialNetworkDetailId);
		}

		/// <summary>
		/// Performs a select statement on table "hrm.employee_social_network_details" producing a paged result of 10.
		/// </summary>
        /// <param name="catalog">The name of the database on which queries are being executed to.</param>
		/// <returns>Returns the first page of collection of "EmployeeSocialNetworkDetail" class.</returns>
		public IEnumerable<MixERP.Net.Entities.HRM.EmployeeSocialNetworkDetail> GetPagedResult(string catalog)
		{
			...
			return Factory.Get<MixERP.Net.Entities.HRM.EmployeeSocialNetworkDetail>(catalog, sql);
		}
		...
		public IEnumerable<MixERP.Net.Entities.HRM.EmployeeSocialNetworkDetail> GetPagedResult(string catalog, long pageNumber)
		{
			long offset = (pageNumber -1) * 10;
			const string sql = "SELECT * FROM hrm.employee_social_network_details ORDER BY employee_social_network_detail_id LIMIT 10 OFFSET @0;";
				
			return Factory.Get<MixERP.Net.Entities.HRM.EmployeeSocialNetworkDetail>(catalog, sql, offset);
		}
	}
}